// Add this at the bottom of your GoalA component, after the daily goals list
'use client'
...
export default function GoalA() {
    // Track checked for goal of the week
    const [weekChecked, setWeekChecked] = useState(false);
    // Track checked for each day
    const [checks, setChecks] = useState(Array(7).fill(false));

    const handleCheck = (idx: number) => {
        setChecks(prev => prev.map((val, i) => i === idx ? !val : val));
    };

    // Penalty for goal of the week
    const weekPenalty = weekChecked ? 0 : 1;
    // Penalty for goal a of the day
    const dayPenalty = Math.max(0, 7 - checks.filter(Boolean).length - 2);
    // Total penalty
